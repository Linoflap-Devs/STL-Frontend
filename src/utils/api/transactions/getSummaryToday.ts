...
const fetchSummaryToday = async (queryParams: Record<string, any>) => {
    try {

        // Retirieving authToken but not using it in the request, add it to the request headers if required.
        const token = localStorage.getItem("authToken");

        const response = await axiosInstance.get("/transactions/getTodaySummary", {
            params: queryParams,
            headers: {  
        ...
        console.log(`Response Data (fetchSummaryToday): ${JSON.stringify(response.data)}`);
        return response.data;
    } catch (error) {
        if (error instanceof AxiosError) {
            console.error("Error fetching Get Summary Today:", error.response?.data?.message || error.message);
...
export default fetchSummaryToday;
