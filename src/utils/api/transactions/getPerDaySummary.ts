...
const fetchPerDaySummary = async (queryParams: Record<string, any>) => {
    try {

        // Retirieving authToken but not using it in the request, add it to the request headers if required.
        const token = localStorage.getItem("authToken");

        const response = await axiosInstance.get("/transactions/getPerDay", {
            params: queryParams,
            headers: {  
        ...
        console.log(`Response Data (Per Day Summary)): ${JSON.stringify(response.data)}`);
        return response.data;
    } catch (error) {
        if (error instanceof AxiosError) {
            console.error("Error fetching Per Day Summary:", error.response?.data?.message || error.message);
...
export default fetchPerDaySummary;
