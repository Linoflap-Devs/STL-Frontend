...
/**
 * A service function to fetch data from the Test API endpoint.
...
/*
    * Add type gradually as our API respose become clearer:
    ...
    const response = await getTestData('/some/endpoint');
    ...
    const typedResponse = await getTestData<MyResponseType>('/typed/endpoint');
*/
...
const getTestData = async <T = unknown>(
    endpoint: string,
    ...
    try {
        ...
        return {
            success: true,
            message: "GET Request on Test Data Succeeded",
            data: response.data
        }
    } catch (error) {
