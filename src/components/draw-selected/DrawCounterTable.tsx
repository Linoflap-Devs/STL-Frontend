...
const generateRandomNumbers = (length: number, min = 1, max = 90) => {
...
const DrawCounterTablePage = () => {
  ...
  useEffect(() => {
    const randomNumbers = generateRandomNumbers(32, 1, 90);
    ...
    <div className="bg-[#171717] rounded-xl text-white w-auto inline-block">
      <div className="grid grid-cols-8">
        {counterDraw.map((num, index) => (
          <div
            key={index}
            className={`relative p-7 flex flex-col items-center justify-center text-sm aspect-square
              ${!isRightEdge(index) ? "border-r" : ""}
              ${!isBottomEdge(index) ? "border-b" : ""}
              border-[#D5D5D5]`}
          >
            <div className="absolute top-1 left-1 text-sm text-gray-300 font-semibold p-1">{index + 1}</div>
            <div className="text-white-400 font-bold text-3xl">{num}</div>
          </div>
        ))}
      </div>
    </div>
