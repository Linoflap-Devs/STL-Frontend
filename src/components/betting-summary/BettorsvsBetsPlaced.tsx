...
const BettorsvsBetsPlacedPage: React.FC<BettorsvsBetsPlacedPageProps> = ({
  data,
}) => {
  console.log(
    "Fetched Data from BettorsvsBetsPlaced:",
  ...
  useEffect(() => {
    ...
    const processData = () => {
      ...
      setBettorsData(
        allRegions.map((region) => regionMap[region]?.bettors || 0)
      );
      setBetsPlacedData(
        allRegions.map((region) => regionMap[region]?.bets || 0)
      );
    };
  ...
  return (
    ...
    >
      <Box sx={{ paddingX: 3 }}>
        ...
        <Divider
          sx={{ backgroundColor: "#B3B3B3", opacity: 1, height: "2px", mt: 1 }}
        />
      </Box>
      ...
      <Box sx={{ height: 270, width: "100%", flexGrow: 0, minWidth: 0 }}>
        <BarChart
          ...
          xAxis={[
            { scaleType: "band", data: regions, label: "PHILIPPINE REGIONS" },
          ]}
          yAxis={[{ label: "AMOUNT" }]}
          series={[
