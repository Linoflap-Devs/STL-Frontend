...
import { Typography, Box, Stack, Divider, Button, Menu, MenuItem } from "@mui/material";
import FilterListIcon from '@mui/icons-material/FilterList';

// Custom Legend
const CustomLegend = () => (
    <Stack direction="row" spacing={4} justifyContent="right" sx={{ mt: 2, mr: 4 }}>
        ...
        <>
            <Box sx={{ mt: 4, mb: 1.5, }}>
                ...
                <Menu sx={{ mt: 1, }} anchorEl={anchorEl} open={open} onClose={() => handleClose(selectedFilter)}>
                    {["Specific Date", "Date Duration",].map((filter) => (
                        <MenuItem
                            key={filter}
            ...
            <Box sx={{ paddingTop: 7, paddingBottom: 4, paddingLeft: 2, backgroundColor: "#1F2937" }}>
                <Box sx={{ paddingX: 3, }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'right', marginRight: 4, borderRadius: '8px' }}>
                    <Button
                        sx={{
                            fontSize: '12px', textAlign: "left", textTransform: 'none', backgroundColor: "#2563EB", mb: 0.7, paddingX: 4, paddingY: 0.9, width: 'auto', "&:hover": {
                                backgroundColor: '#1D4ED8',
                            },
                        }}
                        variant="contained"
