...
const ChartBettorsvsBetsPlacedSummary = () => {
  const [data, setData] = useState<{ draw: string; bettors: number; bets: number }[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log("Result Data from BettorsvsBetsPlacedChart: " + JSON.stringify(res.data, null, 2));

        if (res.success && Array.isArray(res.data)) {
          // Aggregate data by GameTypeId
          const aggregatedData: Record<number, { bettors: number; bets: number }> = {};

          res.data.forEach((item: { GameTypeId: number; TotalBettors: number; TotalBetsPlaced: number }) => {
          ...
          const formattedData = [
            { draw: "First Draw", bettors: aggregatedData[1]?.bettors || 0, bets: aggregatedData[1]?.bets || 0 },
            { draw: "Second Draw", bettors: aggregatedData[2]?.bettors || 0, bets: aggregatedData[2]?.bets || 0 },
            { draw: "Third Draw", bettors: aggregatedData[3]?.bettors || 0, bets: aggregatedData[3]?.bets || 0 },
          ];
        ...
        console.log("Error loading BettorsvsBetsPlacedSummary: " + (error as Error).message);
      }
    };
  ...
  const maxX = Math.max(70, ...data.map((item) => item.bettors / 10000), ...data.map((item) => item.bets / 100000));
    ...
    <Box sx={{ backgroundColor: "#171717", padding: "1rem", borderRadius: "8px", paddingBottom: "2rem" }}>
      ...
      <Box sx={{ height: 270 }}>
        <BarChart
          ...
          xAxis={[{ label: "Amount (in 100,000 units)", scaleType: "linear", min: 0, max: maxX, tickValues: xAxisTicks } as any]}
        />
      </Box>
    </Box>
  );
